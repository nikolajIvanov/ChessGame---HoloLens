...
public class H_King : H_Piece
{
    ...
    private H_Piece leftRook;
    private H_Piece rightRook;
    ...
    private void AssignCastlingMoves()
    {
        ...
        if (!hasMoved)
        {
            leftRook = GetPieceInDirection<H_Rook>(team, Vector2Int.left);
            if (leftRook && !leftRook.hasMoved)
            {
                leftCastlingMove = occupiedSquare + Vector2Int.left * 2;
                avaliableMoves.Add(leftCastlingMove);
            }
            rightRook = GetPieceInDirection<H_Rook>(team, Vector2Int.right);
    ...
    private H_Piece GetPieceInDirection<T>(H_TeamColor team, Vector2Int direction)
    {
        for (int i = 1; i <= H_Board.BOARD_SIZE; i++)
        {
            Vector2Int nextCoords = occupiedSquare + direction * i;
            H_Piece piece = board.GetPieceOnSquare(nextCoords);
